...
export function loadUserPreferences(): UserPreferences {
  ...
  try {
    const stored = JSON.parse(localStorage.getItem(PREFERENCES_KEY) ?? "null") as Partial<UserPreferences> | null;
    return stored ? { ...defaultUserPreferences, ...stored, version: 1 } : defaultUserPreferences;
  } catch {
