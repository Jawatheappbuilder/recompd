...
function ProfilePage() {
  ...
  return <Screen><SettingsHeader title="Edit Profile" /><div className="space-y-4">
    ...
    <SettingsSection title="Details"><div className="space-y-4 py-4"><label className="block text-xs font-bold text-muted-foreground">Name<Input value={name} onChange={(event) => setName(event.target.value)} className="mt-2 h-11 rounded-xl bg-secondary text-foreground" /></label><label className="block text-xs font-bold text-muted-foreground">Height (cm)<Input type="number" inputMode="numeric" min="80" max="250" value={height} onChange={(event) => setHeight(event.target.value)} placeholder="Not set" className="mt-2 h-11 rounded-xl bg-secondary text-foreground" /></label><CompactChoice label="Gender" value={gender || "Prefer not to say"} options={genders} onChange={setGender} className="grid-flow-row grid-cols-2" /></div></SettingsSection>
