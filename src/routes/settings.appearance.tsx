...
function AppearancePage() {
  ...
  return <Screen><SettingsHeader title="Appearance" /><SettingsSection title="Theme">{choices.map(({ id, label, icon: Icon }) => <Button key={id} variant="ghost" className="h-14 w-full justify-start rounded-none px-0 hover:bg-transparent" onClick={() => setPreferences({ ...preferences, theme: id })}><span className="grid size-8 place-items-center rounded-lg bg-secondary text-muted-foreground"><Icon className="size-4" /></span><span className="flex-1 text-left text-sm font-bold">{label}</span>{preferences.theme === id ? <Check className="size-4 text-primary" /> : null}</Button>)}</SettingsSection></Screen>;
}
