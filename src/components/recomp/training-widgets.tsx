...
import { useScheduledWorkouts } from "@/lib/workout-storage";
...
export function WeekTracker({ start, trainedDays, scheduledByDay, startsOn }: { start: number; trainedDays: Set<string>; scheduledByDay: Map<string, string>; startsOn: "Monday" | "Sunday" }) {
  ...
  return <div className="grid min-w-0 grid-cols-7 gap-1">{dayLetters[startsOn].map((label, index) => {
    const key = dayKey(start + index * DAY_MS + DAY_MS / 2);
    const complete = trainedDays.has(key);
    const scheduledId = !complete ? scheduledByDay.get(key) : undefined;
    const isToday = key === today;
    const circle = <div className={cn("grid size-6 place-items-center rounded-full border text-[0.62rem] font-black min-[390px]:size-7 min-[390px]:text-[0.65rem]", complete && "border-primary bg-primary text-primary-foreground", scheduledId && "border-primary/50 bg-accent text-primary", !complete && !scheduledId && isToday && "border-primary text-primary", !complete && !scheduledId && !isToday && "border-border bg-secondary text-muted-foreground")}>{complete ? <Check className="size-3.5" strokeWidth={3} /> : new Date(start + index * DAY_MS + DAY_MS / 2).getDate()}</div>;
    return <div className="flex min-w-0 flex-col items-center gap-1.5" key={`${label}-${index}`}><span className="text-[0.62rem] font-bold text-muted-foreground">{label}</span>{scheduledId ? <Link to="/scheduled/$id" params={{ id: scheduledId }} aria-label={`View scheduled workout for ${key}`}>{circle}</Link> : circle}</div>;
  })}</div>;
}
