...
export function ProgressRing({ value = 75, current = 3, target = 4, size = 86 }: { value?: number; current?: number; target?: number; size?: number }) {
  ...
  return <div className="relative shrink-0" style={{ width: size, height: size }}>
    ...
    <svg className={cn("-rotate-90", celebrating && "motion-safe:animate-[pulse_900ms_ease-in-out_2]")} viewBox="0 0 86 86" aria-label={`${current} of ${target} workouts complete`}><circle cx="43" cy="43" r={radius} fill="none" stroke="var(--color-track)" strokeWidth="7"/><circle className={cn("progress-stroke transition-colors duration-500", complete && value >= 100 && "stroke-emerald-500")} cx="43" cy="43" r={radius} fill="none" stroke="var(--color-primary)" strokeWidth="7" strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={circumference * (1 - value / 100)}/></svg>
